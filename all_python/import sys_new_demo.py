    ...
    def update_plot(self, data, tracks):
        ...
        if n_tracks == 0:
            ...
            for idx, (ax, track) in enumerate(zip(axes, tracks)):
                ...
                # Sync the limits of the twin axis with the primary axis
                if twin_ax:
                    twin_ax.set_xlim(ax.get_xlim())

                # Add legend in the upper right (above the graph)
                if lines_list:
                    ax.legend(lines_list, [line.get_gid() for line in lines_list], loc='upper right', bbox_to_anchor=(1, 1.185), ncol=1)
                ...
                if track.x_min.text():
                    try:
                        ...
                        if twin_ax:
                            twin_ax.set_xlim(ax.get_xlim())  # Sync limits
                    except ValueError:
                        pass
                if track.x_max.text():
                    try:
...
class WellLogViewer(QMainWindow):
    def __init__(self):
        ...
        self.figure_widgets = {}
        self.initUI()
        self.setWindowIcon(QIcon('ONGC_Logo.png'))
...
app.setStyleSheet(loadStyleSheet("darkmode.qss"))
